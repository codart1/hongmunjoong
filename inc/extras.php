...
if ( ! function_exists( 'dazzling_featured_custom_header' ) ) :
  /**
  ** create custom header for
  */
function dazzling_featured_custom_header() {
  echo '<div class="container"><div style="width:230px;height:150px;background:#c25448">
  sdf
  </div></div.';
}
endif;
...
function dazzling_featured_slider() {
    if ( is_front_page() && of_get_option('dazzling_slider_checkbox') == 1 ) {
      ...
      echo '<div class="flexslider">';
          ...
          $count = of_get_option('dazzling_slide_number')-1;
        ...
        echo '<ul class="slides">';
            ...
            if ( $count && $slidecat ) {
            $query = new WP_Query( array( 'cat' => $slidecat, 'posts_per_page' => $count, 'orderby' => "ID", 'order' =>'ASC' ) );
            if ($query->have_posts()) :
              while ($query->have_posts()) : $query->the_post();

              echo '<li>';
                if ( (function_exists('has_post_thumbnail')) && (has_post_thumbnail()) ) :
                  echo '<div style="background-position:center;height:520px;display:block;background-image:url('.get_the_post_thumbnail_src(get_the_post_thumbnail()).')"></div>';
                endif;
